...
public class KVCommunicationClient {
  ...
  public void sendMessage(KVMessage msg) throws IOException {
    ...
    logger.debug(
        String.format(
            "SEND %s > key: %s || value: %s",
            msg.getStatus().toString(), msg.getKey(), msg.getValue()));

    DebugHelper.logFuncExit(logger);
  }

  /**
   * Receive message from input stream, parse the bytes, and assemble a readable message.
   *
