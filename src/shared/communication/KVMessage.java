...
public class KVMessage implements IKVMessage {
  ...
  /**
   * Split a message into its type, key, and value. Then, assign them to associated class members.
   *
  ...
  private boolean isMessageLengthValid(String key, String value) throws Exception {
    ...
    if (keySizeBytes > MAX_KEY_SIZE_BYTES) {
      errorMsg =
          String.format(
              "Key length of %s bytes exceeds limit of %s bytes.",
              keySizeBytes, MAX_KEY_SIZE_BYTES);
      logger.error(errorMsg);
      throw new Exception(errorMsg);
    }
    ...
    if (valueSizeBytes > MAX_VALUE_SIZE_BYTES) {
      errorMsg =
          String.format(
              "Value length of %s bytes exceeds limit of %s bytes.",
              valueSizeBytes, MAX_VALUE_SIZE_BYTES);
      logger.error(errorMsg);
      throw new Exception(errorMsg);
    }
